...
fun createGpxDocument(movimentosData: MutableList<Movimento>, activityName: String): String {
    ...
    if (movimentosData.isNullOrEmpty()) {
        return "Array Zerado!"
    }
    ...
    metaDateTime.appendChild(doc.createTextNode(movimentosData[0].timestamp.toString()))
    ...
    for (movimento in movimentosData) {
        ...
        trkpt.setAttribute("lat", movimento.latitude.toString())
        trkpt.setAttribute("lon", movimento.longitude.toString())

        val trkptDateTime: Element = doc.createElement(("trkptDateTime"))
        trkptDateTime.appendChild(doc.createTextNode(movimento.timestamp.toString()))
        trkpt.appendChild(trkptDateTime)

        val trkptAccelData: Element = doc.createElement(("trkptAccelData"))
        trkptAccelData.setAttribute("AxisX", movimento.accelX.toString())
        trkptAccelData.setAttribute("AxisY", movimento.accelY.toString())
        trkptAccelData.setAttribute("AxisZ", movimento.accelZ.toString())
        trkpt.appendChild(trkptAccelData)
